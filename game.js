...
let mysteryDeck=[], mysteryTurn=1, mysteryFlipped=[], mysteryLocked=false, owned=[[],[]], duelHp={}, duelChoices=[null,null];
function startMystery(){ hideAll(); document.querySelector("#mystery-screen").hidden=false; mysteryTurn=1; mysteryFlipped=[]; mysteryLocked=false; owned=[[],[]]; document.querySelector("#duel-panel").hidden=true; mysteryDeck=flyers.flatMap(c=>[c,c]).concat([joker,joker]).sort(()=>Math.random()-.5); updateMysteryTurn(); renderMystery(); renderOwned(); }
...
function finishMysteryDraft(){
  ...
  const ownedIds=owned.flat().map(c=>c.id);
  let remaining=[...new Map([...flyers,joker].filter(c=>!ownedIds.includes(c.id)).map(c=>[c.id,c])).values()];
